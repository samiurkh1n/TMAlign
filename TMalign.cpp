...
void print_help(char *arg)
{
  ...
  cout << " ***************************************************************************************************" << endl
       ...
       << " ***************************************************************************************************" << endl;	
  cout << endl
       << " Usage: " << arg << " PDB1.pdb PDB2.pdb [Options]" << endl
       << "        " << arg << " PDB1.pdb List.txt -dir PDB_dir/ [Options]" << endl
       << " Options:" << endl
...
int main(int argc, char *argv[])
{
  ...
  clock_t t1, t2, alignment_t1, alignment_t2;
  t1 = clock();
  ...
  if( !B_opt )
    {
      cout << "Please provide structure or list B" << endl;
      exit(EXIT_FAILURE);
    }		
  if( !A_opt )
    {
  ...
  if( dir_opt )
	...
	{
	  // Each element is a path to the protein starting at the directory of TMAlign
	  proteins.push_back(argv[4] + protein + ".pdb");
	}
    }
  if( a_opt )
    {
  ...
  for (size_t protein_idx = 0; protein_idx < proteins.size(); ++protein_idx) {
    ...
    if (dir_opt) {
      ...
      float alignment_time = ((float)alignment_t2 - (float)alignment_t1)/CLOCKS_PER_SEC;
      printf("\nRunning time for alignment is %5.2f seconds\n", alignment_time);
    }
