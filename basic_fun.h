...
int read_PDB(char *filename, double **a, char *seq, int *resno, int **nres, int ter_value)
{
  int i=0;
  string line, str;    
  string atom ("ATOM "); 
  ...
  if (fin.is_open())
    {
      bool bGoRead = true;
      bool firstTerHit = false;
      while (fin.good() && bGoRead)
        {
	  getline(fin, line);
	  if (i > 0 && (line.compare(0, 3, ter) == 0) && (ter_value == 0 || (ter_value == 1 && firstTerHit)))
	    {
	      bGoRead = false;
	    }
	  else if (i > 0 && (line.compare(0, 3, ter) == 0) && ((ter_value == 1 && !firstTerHit)))
	    {
	      firstTerHit = true;
	      continue;
	    }
	  else if (i > 0 && (line.compare(0, 3, end) == 0))
	    {
	      bGoRead = false;
	    }
	  else
...
int read_PDB_fullatom(char *filename, double **a, char *seq, int *resno, int *ia, char **aa, char **ra, int *ir, double **xyza, int **nres, string **atom0, char* ains, char* ins, int& atomlen, int ter_value)
{
  char dest[1000];
