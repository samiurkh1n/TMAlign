...
void load_PDB_allocate_memory(char *xname, char *yname, int ter_value)
{    
  ...
  if (!o_opt)
    {
      xlen = read_PDB(xname, xa, seqx, xresno, nres1, ter_value);// Get exact length
      ylen = read_PDB(yname, ya, seqy, yresno, nres2, ter_value);
      ...
      for (int i = 0; i < NMAX; i++)// Initialization
	{
      ...
      xlen = read_PDB_fullatom(xname, xa, seqx, xresno, ia1, aa1, ra1, ir1, xyza1, nres1, atom1, ains1, ins1, atomxlen, ter_value);
      ylen = read_PDB_fullatom(yname, ya, seqy, yresno, ia2, aa2, ra2, ir2, xyza2, nres2, atom2, ains2, ins2, atomylen, ter_value);
      minlen = min(xlen, ylen);
    }
